...
unsigned char uart1_buffer_rx_load(void)
{
  ...
  if(!uart1_status.buffer_rx_full)
  {
	...
	// buffer (no loopback)
    if(uart1_rs485_enable && uart1_rs485_tx_enable)
	  return 0;
...
unsigned char uart2_buffer_rx_load(void)
{
  ...
  if(!uart2_status.buffer_rx_full)
  {
	...
	// if I'm in rs485 mode and I sent something then I wont read it from 
	// buffer (no loopback)
    if(uart2_rs485_enable && uart2_rs485_tx_enable)
	  return 0;
...
/**************************************************
* Function name		: unsigned char uart1_error_handle(unsigned char *buffer, unsigned char *length)
...
**************************************************/
unsigned char uart1_error_handle(unsigned char *error_message, unsigned char *length)
{
...
/**************************************************
* Function name		: unsigned char uart2_error_handle(unsigned char *buffer, unsigned char *length)
* 	return			: 0 - no error occurred
*					  1 - error message loaded
*	buffer			: pointer to a buffer where store the error message
*   length			: char where to store the length of the message 
...
*					  buffer must be 30 byte
**************************************************/
unsigned char uart2_error_handle(unsigned char *error_message, unsigned char *length)
{
