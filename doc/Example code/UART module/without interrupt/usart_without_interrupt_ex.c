/***************************************************
* FileName:        usart_without_interrupt_ex.c
...
*
* First written on 20/10/2012 by Luca Lucci.
*
* Module Description:
* In this example it has been described the use of the usart module. The
* purpouse of this code is receive a byte of data and performe an echo by
* polling RC2IF flags.
*
* Note:
...
void main(void)
{
  ...
  while(1) 
  {
    ...
    // check if there's something to read and load rx buffer
    if(uart2_buffer_rx_load())
    {
      // read rx buffer and store the data into a temp variable
	  if(uart2_buffer_read(temp))
	  {
	    // I want to send back the data read, so load tx buffer and
		// send the data on usart

		if( uart2_buffer_tx_load(temp))
          uart2_buffer_send();
      }
