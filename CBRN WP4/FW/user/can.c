...
struct CAN_MESSAGE_STRUCT
{
  ...
  unsigned char error_invalid_msg;
...
* 	config_interrupt : 		enable can interrupt such as tx, rx, error and invalid 
*					   		message. See PIE5 and TXBIE register.
...
* Notes				: -
**************************************************/
void can_init_interrupt(unsigned char config_interrupt, unsigned char config_prog_interrupt)
{
...
/* remove
  if(tx_rx_buffer)
  	BSEL0 |= 2^(prog_buffer + 2);	//set the selected flag
  else
    BSEL0 &= !(2^(prog_buffer + 2));	//clear the selected flag
*/
  // return to previouse op mode
  if(old_op_mode != CANSTATbits.OPMODE)
    can_op_select(old_op_mode);
...
* Function name		: unsigned char can_init_baudrate_set(unsigned char SJW, unsigned char PRSEG,
												 ...
												 unsigned char clock, unsigned char baud_rate)
...
*	PS1				: ps1 value in Tq
*	PS2				: ps2 value in Tq
* 	clock			: clock value with pll
*	baud_rate		: n times of 125kHz, where n is a power of twos
*
* Created by		: Luca Lucci
* Date created		: 22/10/12
* Description		: 
...
unsigned char can_init_baudrate_set(unsigned char SJW, unsigned char PRSEG,
									unsigned char PS1, unsigned char PS2, 
									unsigned int clock, unsigned char baud_rate)
{
  unsigned int brp;
  unsigned char number_of_tq;
  ...
  // Nominal bit time is programmable form a minimum of 8 Tq to a maximum of 25
  ...
  if( PRSEG > 0 && PRSEG < 9 && PS1 > 0 && PS1 < 9 && PS2 > 1 && PS2 < 9 && SJW > 0 && SJW < 5)
  {
    if( (PRSEG + PS1) >= PS2 && PS2 > SJW)
      number_of_tq = SJW + PRSEG + PS1 + PS2;
    else
      return 0;
  }
  else
  ...
  if(number_of_tq > 7 && number_of_tq < 26)
  {
    ...
    BRGCON1bits.SJW = SJW - 1;		// 1 Tq
    BRGCON2bits.PRSEG = PRSEG - 1;	// 3 Tq
    BRGCON2bits.SEG1PH = PS1 - 1;	// 2 Tq
    BRGCON3bits.SEG2PH = PS2 - 1;	// 2 Tq
  
    number_of_tq = SJW + PRSEG + PS1 + PS2;

    brp = ((clock * 4) / ((1 << baud_rate) * number_of_tq)) - 1;
...
{
  if(filter > 15 || !(BSEL0 & 2^(prog_buffer+2)))
	return 0;
  else
  {
...
*	return			: 0 - Failure: "Mode 0 not implemented. Please select 
*						  Mode1 - Enhanced legacy mode"
*					  1 - Success
* 	mode			: next mode
* Created by		: Luca Lucci
* Date created		: 21/10/12
...
/**************************************************
* Function name		: void can_read(CAN_MESSAGE *can_message, unsigned char buffer_index)
...
* Notes				: -
**************************************************/
void can_read(CAN_MESSAGE *can_message, unsigned char buffer_index)
{
