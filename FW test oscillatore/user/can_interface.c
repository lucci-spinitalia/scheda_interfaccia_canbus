...
struct CAN_STATUS
{
  ...
  unsigned buffer_rx_error_overflow		:1; // rx buffer full and can't receive data anymore
...
//flag to indicate that a buffer rx has been loaded
volatile unsigned char can_buffer_rx_loaded; 

unsigned char can_mode = 0; //initialized by init routine
...
/**************************************************
* Function name		: unsigned char can_init(unsigned char mode, unsigned char global_interrupt, 
*					   unsigned char prog_interrupt, unsigned char prog_buffer_mode, 
*					   unsigned char* sync_param)
* 	result			: 0 - failure
*					: 1 - success
*
*   mode			: Functional Mode:
*					  0 - Legacy Mode
*					  1 - Enhanced Legacy Mode
...
**************************************************/
unsigned char can_init(unsigned char mode, unsigned char global_interrupt, 
					   unsigned char prog_interrupt, unsigned char prog_buffer_mode, 
					   unsigned char* sync_param)
{
  ...
  // See cap. 27.4 (pag. 439) of datasheet for mode details
  can_mode = mode;

  if(!can_mode_select(can_mode))
  {
  ...
  can_init_interrupt(can_mode, global_interrupt, prog_interrupt);
  ...
  ****************************************************/
  ...
  for( i = 0; i < 16; i++)
  {
    if(can_init_filter_buffer(i, i >> 1) == 0)
      return 0;
  }
...
/**************************************************
* Function name		: unsigned char can_buffer_rx_load()
*	return			: 0 = failure
*					  1 = success
* Created by		: Luca Lucci
* Date created		: 31/10/12
...
unsigned char can_buffer_rx_load(void)
{
  ...
  if(!can_status.buffer_rx_full)
  {
    ...
    // If can bus isn't in mode 2 (FIFO) I need to manage the buffer
    // priority
    if(can_buffer_rx_loaded && (can_mode != 2))
    {
      for(buf_test = 0; buf_test < 8; buf_test++)
      {
        //start from lower buffer
        if((can_buffer_rx_loaded >> buf_test) & 0x01)
        {
	      //Read the message
	      can_read(&can_buffer_rx[can_buffer_rx_wr_ptr], buf_test, 1);
          ...
          can_buffer_rx_loaded -= (0x01 << buf_test);

          if(can_buffer_rx_loaded == 0)
	        break;

		  //Check if there's enough space to store another message, else exit
          if(can_status.buffer_rx_full)
			break;
        }
      }
...
/**************************************************
* Function name		: unsigned char can_get_rx_data_size(void)
*	return			: number of byte to read through can_buffer_rx_load
* Created by		: Luca Lucci
* Date created		: 31/10/12
...
void can_isr(void)
{
  ...
  if(PIR5bits.ERRIF && PIE5bits.ERRIE)
  {
	...
	// All if statement but RXB1OVFL are used for debug
...
/**************************************************
* Function name		: void can_buffer_rx_loaded_update()
* 
*	mode			: 0 - Legacy Mode
*					  1 - Enhanced Mode
*					  2 - Enhanced FIFO Mode
...
* Notes				: -
**************************************************/
void can_buffer_rx_loaded_update()
{
  unsigned char i;
  ...
  switch(can_mode)
  {
    ...
    case 2:    
      // Get the pointer to the register to read
      can_buffer_rx_loaded = CANCON & 0x07;

      // If there's no message, exit
      if((*RXBn_list.RXBnCON[can_buffer_rx_loaded] & 0x80) == 0)
        break;

      // check if the register has to be read. If there's no data anymore, I have to wait
      // the count down 
      //while(((*RXBn_list.RXBnCON[can_buffer_rx_loaded] & 0x80) != 0) /*|| (count < 200)*/)
      if(((*RXBn_list.RXBnCON[can_buffer_rx_loaded] & 0x80) != 0) /*|| (count < 200)*/)
      {
        //if(count < 200)
        //  count++;
	    ...
	    //Read the message
	    can_read(&can_buffer_rx[can_buffer_rx_wr_ptr], can_buffer_rx_loaded, 1);
...
**************************************************/
unsigned char can_error_handle()
{
  ...
  {
	can_status.buffer_rx_error_overflow = 0;
    return BUFFER_RX_ERROR_OVERFLOW;
  }
...
/**************************************************
* Function name		: unsigned char can_error_translate(unsigned char *error_message, unsigned char code, unsigned char *length)
* 	return			: 0 - no error occurred
...
**************************************************/
unsigned char can_error_translate(unsigned char *error_message, unsigned char code, unsigned char *length)
{
  switch(code)
  {
